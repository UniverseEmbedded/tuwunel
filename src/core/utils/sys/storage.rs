...
mod unix_storage {
	...
	use super::{MultiDevice, MultiQueue, Queue};
...
mod windows_storage {
	...
	pub fn md_discover(_path: &Path) -> MultiDevice {
		MultiDevice::default()
	}
	...
	pub fn name_from_path(path: &Path) -> Result<String> {
		...
		if path_str.len() >= 2 {
			let drive_letter = path_str.chars().next().unwrap_or('C');
			if drive_letter.is_ascii_alphabetic() {
				return Ok(format!("\\\\?\\Volume{{}}\\{}:", drive_letter));
			}
		}

		Err(Error::new(NotFound, "Cannot determine device name on Windows").into())
	}
}

#[cfg(unix)]
pub use unix_storage::{md_discover, name_from_path};

#[cfg(windows)]
pub use windows_storage::{md_discover, name_from_path};
...
pub fn name_from_path(path: &Path) -> Result<String> {
	use std::io::{Error, ErrorKind::NotFound};
	Err(Error::new(NotFound, "Storage discovery not supported on this platform").into())
}
